...
import QuizSettingsModal from '../components/QuizSettings/QuizSettingsModal';
import { QuizSettings } from '../types';

const Home: React.FC = () => {
  const navigate = useNavigate();
  const { categories, startQuiz, hasSavedQuiz, resumeQuiz } = useQuizStore();
  ...
  const handleStartQuizWithSettings = async (settings: QuizSettings) => {
    await startQuiz(settings);
    navigate(`/quiz/${settings.category}`);
  };
      ...
      >
        ...
        {hasSavedQuiz() && (
          <motion.div variants={itemVariants}>
            ...
            >
              <Box display="flex" alignItems="center" justifyContent="center" mb={2}>
                <PlayArrow sx={{ fontSize: 40, color: '#10b981', mr: 2 }} />
                <Typography variant="h3" sx={{ fontWeight: 600, color: '#10b981' }}>
                  Resume Your Quiz
                </Typography>
              </Box>
              <Typography variant="body1" sx={{ mb: 3, color: 'rgba(255, 255, 255, 0.8)' }}>
                You have a paused quiz waiting! Continue from where you left off.
              </Typography>
              <motion.div
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
              ...
              <Grid size={{ xs: 12, sm: 6, md: 4 }} key={category.id}>
                ...
                >
                  <Card
                    ...
                    onClick={() => handleCategoryClick(category.name, category.displayName)}
                  >
                    ...
                    <CardActions sx={{ p: 2, pt: 0 }}>
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleCategoryClick(category.name, category.displayName);
                        }}
                      >
                        Start Quiz
                      </Button>

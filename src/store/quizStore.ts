  ...
  persist(
    (set, get) => ({
      ...
      submitAnswer: (answer, timeSpent) => {
        ...
        if (isCorrect) {
          // Calculate base score per question based on total questions
          // Total max score will be 1000, distributed evenly across all questions
          const totalQuestions = currentQuiz.questions.length;
          const baseScorePerQuestion = Math.round(1000 / totalQuestions);
          
          // Time bonus: up to 20% of base score for quick answers
          const timePercentage = currentQuiz.timeLeft / currentQuiz.settings.timeLimit;
          const timeBonus = Math.round(baseScorePerQuestion * 0.2 * timePercentage);
          
          // Difficulty multiplier (but capped to not exceed max)
          const difficultyMultiplier = question.difficulty === 'hard' ? 1.15 : 
                                       question.difficulty === 'medium' ? 1.1 : 1.0;
          
          // Calculate final points (ensuring we don't exceed reasonable limits)
          const rawPoints = Math.round((baseScorePerQuestion + timeBonus) * difficultyMultiplier);
          const maxPointsPerQuestion = Math.round(1200 / totalQuestions); // Allow some bonus but cap it
          const points = Math.min(rawPoints, maxPointsPerQuestion);
          
          set({
            currentQuiz: {
              ...currentQuiz,
              score: currentQuiz.score + points,
              correctAnswers: (currentQuiz.correctAnswers || 0) + 1
            }
          });
      ...
      previousQuestion: () => {
        ...
        const prevIndex = currentQuiz.currentQuestionIndex - 1;
      ...
      abandonQuiz: () => {
        set({ currentQuiz: null, savedQuiz: null });
      ...
      pauseQuiz: () => {
        ...
        set({ 
          savedQuiz: pausedQuiz,
          currentQuiz: null 
        });
        toast.success('Quiz paused. You can resume later! ⏸️');
      },
      ...
      resumeQuiz: () => {
        ...
        const now = Date.now();
        const pauseTime = savedQuiz.pausedAt ? 
          now - savedQuiz.pausedAt : 0;
        
        const resumedQuiz = {
          ...savedQuiz,
          isPaused: false,
          pausedAt: undefined,
          totalPauseTime: (savedQuiz.totalPauseTime || 0) + pauseTime
        };
        
        set({ 
          currentQuiz: resumedQuiz,
          savedQuiz: null 
        });
        toast.success('Quiz resumed! Welcome back! ▶️');
      },
